...
/**
 * <p>This is a utility class used by selectors and DirectoryScanner. The functionality more
 * properly belongs just to selectors, but unfortunately DirectoryScanner exposed these as
 * protected methods. Thus we have to support any subclasses of DirectoryScanner that may access
 * these methods.</p>
...
public final class SelectorUtil {
    ...
    /**
     * Returns dependency information on these two files. If src has been modified later
     * than target, it returns true. If target doesn't exist, it likewise returns true. Otherwise,
     * target is newer than src and is not out of date, thus the method returns false. It also
     * returns false if the src file doesn't even exist, since how could the target then be out of
     * date.
     *
     * @param src the original file
     * @param target the file being compared against
     * @param granularity the amount in seconds of slack we will give in determining out of
     *        dateness
     *
    ...
    /**
     * Returns dependency information on these two resources. If src has been modified
     * later than target, it returns true. If target doesn't exist, it likewise returns true.
     * Otherwise, target is newer than src and is not out of date, thus the method returns false.
     * It also returns false if the src file doesn't even exist, since how could the target then
     * be out of date.
     *
    ...
    /**
     * "Flattens" a string by removing all whitespace (space, tab, linefeed, carriage
     * return, and formfeed). This uses StringTokenizer and the default set of tokens as
     * documented in the single arguement constructor.
     *
    ...
    public static boolean matchPathPrefix(String name, String[] includes, String[] excludes) {
        ...
        for (int i = 0; i < includes.length; i++) {
            if (matchPatternStart(includes[i], name) && isMorePowerfulThanExcludes(name, excludes)
        ...
        if (match) {
            ...
            for (int i = 0; i < excludes.length; i++) {
                ...
                if (e.equals("**")
                            || (e.endsWith("**")
                                       && matchPath(e.substring(0, e.length() - 2), name, true))) {
                    match = false;
                    break;
                }
            }
    ...
    /**
     * Find out whether one particular include pattern is more powerful than all the
     * excludes. Note:  the power comparison is based on the length of the include pattern and of
     * the exclude patterns without the wildcards. Ideally the comparison should be done based on
     * the depth of the match; that is to say how many file separators have been matched before
     * the first  or the end of the pattern. IMPORTANT : this function should return false "with
     * care".
     *
